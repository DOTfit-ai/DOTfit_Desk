...
def register(mcp):
    ...
    PRINT_LIMIT = 5 	 	# How many to return in the string output
            ...
            try:
                ...
                for xpath_selector, description in selectors_to_try:
                    ...
                    try:
                        ...
                        print(f"[DEBUG] Found {len(link_elements)} elements using selector: {description}")
                        ...
                        for i, link_elem in enumerate(link_elements):
                            ...
                            try:
                                ...
                                # Skip empty URLs or Google internal links
                                ...
                                if url and title and len(url) > 5:
                                    all_links.append({"title": title, "url": url, "rank": len(all_links) + 1})
                                    print(f"[DEBUG] Added link #{len(all_links)}: {title[:50]}...")
                            except Exception as e:
                                print(f"[DEBUG] Error processing link: {str(e)}")
                                continue
                    except Exception as e:
                        print(f"[DEBUG] Selector '{description}' failed: {str(e)}")
                        continue
